...
{
	system("clear");
	printf("(1) Creat a new file to store students' information;\n");
...
// To judge if a file is exist or not.
...
void creat_new_file(void)
{
	...
	printf("Please choose a name for the new file: ");
	char filename[20];
	scanf("%s", filename);
	while(exist(filename))
	{
		printf("\nThe name you choose is already exist!\n");
		printf("please choose another filename: ");
		scanf("%s", filename);
	}
	system("clear");
